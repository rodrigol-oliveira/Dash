...
app.get('/', function (req, res) {
	...
	if (req.query.country !== undefined && req.query.genrer == undefined){
		...
		queryGrp = 'select svod.Group ,sum(case when quarter_year = ' + '"2019 Q1"' + 'then export_data else 0 end) as "2019 Q1" ,sum(case when quarter_year = ' + '"2019 Q2"' + 'then export_data else 0 end) as "2019 Q2",sum(case when quarter_year = ' + '"2019 Q3"' + 'then export_data else 0 end) as "2019 Q3", sum(case when quarter_year = ' + '"2019 Q4"' + 'then export_data else 0 end) as "2019 Q4" from svod where kpi_client = "SVOD and streaming service subscribers" and svod.group in ("Netflix Group", "America Movil Group", "Amazon Group", "WarnerMedia", "Globo Group", "Google Group","Disney Networks") and country = "'+ req.query.country+'" and svod.group in ("Netflix Group", "America Movil Group", "Amazon Group", "WarnerMedia", "Globo Group", "Google Group","Disney Networks")  group by svod.Group order by `2019 Q4` desc ';
